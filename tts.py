...
def tts_batch(d):
    ...
    br = d.get("bitrate") or cfg.get("mp3_bitrate", "128k")
    target_sr = int(cfg.get("batch_sample_rate", 22050))
    ...
    # synth all segments
    for seg in dlg:
        raw_text = _san(seg.get("text") or "")
        if not raw_text:
            continue
        if moder:
            raw_text, _ = moder.filter(raw_text, mode="drop")
            if not raw_text:
                continue
        req_v = (seg.get("voice") or "").strip()
        vid, _ = _resolve_voice_id(req_v)
        tf, wf = _synth_wav_to_path(raw_text, vid, ls, ns, nw, ss, spk)
        rm += [tf, wf]
        u = _resample_to_uniform(wf, target_sr)
        if u != wf:
            rm.append(u)
        seg_uni.append(u)
    ...
    f = _which(cfg.get("ffmpeg_bin", "ffmpeg"))
    if not f:
        # naive join if ffmpeg missing
        merged = tempfile.NamedTemporaryFile(suffix=".wav", delete=False).name
    ...
    src = _norm(merged) if norm else merged
    if src != merged:
        rm.append(src)
    ...
    h = {
        ...
        "X-Format": m,
        "X-Bytes": str(len(b)),
        "X-Segments": str(len(seg_uni)),
        "X-Preset": psel or "",
        "Cache-Control": "no-store",
    }
    ext = "mp3" if m == "audio/mpeg" else "wav"
    h["Content-Disposition"] = f'inline; filename="multi-{rid}.{ext}"'
    return b, m, h
